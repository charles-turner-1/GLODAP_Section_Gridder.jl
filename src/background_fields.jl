function read_bgfield(; section_name::String, var_name::String)
    # This function will go looking for the background field and load it
    if var_name ∉ ["G2tco2", "G2theta", "G2salinity"]
        error("variableName must be either \"G2tco2\", \"G2theta\" or \"G2salinity\"")
    end

    fileName = section_name * "_" * var_name * ".nc"
...
function create_bg_field(; variable::String, sectionName::String, GOSHIP_DIR::String="/home/ct/MATLAB/GO_SHIP", GLODAP_DIR::String="/home/ct/MATLAB/GLODAP", horzCoordinate::String, horzLenFactor::Number=100, vertLenFactor::Number=1, MASK_MATFILE::String="/home/ct/Julia/GLODAP_Easy_Ocean/GOSHIP_MaskStruct.mat")
    ...
    llGrid, prGrid, mask = loadSectionInfo(sectionName, gridDir, MASK_MATFILE)
    _, _, expocodeDir = load_GOSHIP_Directories(GOSHIP_DIR)
    expocodes = listSectionExpocodes(sectionName, expocodeDir)[:, "GLODAP Expocode"]

    vars = load_glodap_vars(["G2theta", "G2tco2", "G2salinity", "G2gamma", "G2latitude", "G2longitude", "G2pressure"], GLODAP_DIR, GLODAP_expocodes=expocodes)
    ...
    # There should be some sort of call to modulo_lon here I think
    # instead of just subtracting 200 from all longitudes carelessly to undo our fuckery.
    if horzCoordinate == "longitude"
        divaGridClim = easyDIVAGrid(variable=zVar, vertVar=y, latLon=x .- lonOffset, horzCoordinate=horzCoordinate, meanValue="scalar", vertGrid=prGrid, horzGrid=llGrid, horzScale=scaleHorz, vertScale=scaleVert, mask=mask, Epsilon=0.1, horzCorrLength=horzLenFactor * clHorz, vertCorrLength=clVert * vertLenFactor)
    elseif horzCoordinate == "latitude"
        divaGridClim = easyDIVAGrid(variable=zVar, vertVar=y, latLon=w, horzCoordinate=horzCoordinate, meanValue="scalar", vertGrid=prGrid, horzGrid=llGrid, horzScale=scaleHorz, vertScale=scaleVert, mask=mask, Epsilon=0.1, horzCorrLength=horzLenFactor * clHorz, vertCorrLength=clVert * vertLenFactor)
    end
...
function writeBackgroundField(; variable::Matrix{Float64}, sectionName::String, variableName::String, outputPathStr::String="/home/ct/Julia/GLODAP_Easy_Ocean/BackgroundFields/")
